...
to go
  ...
  [
    ; let Waiting-Pilgrims (count turtles with [in-queue = true])
    ; let Pilgrims-Doing-Stoning (count turtles with [wait-time > 0])
    ; file-print (word ticks ","  Pilgrims-In-Jamarat"," Waiting-Pilgrims"," Pilgrims-Doing-Stoning","Congestion","CongestionPC","Pilgrims-around-pillars","Throughput )
    file-close
    stop
   ]
end
...
;   if Go-Straight and (Stoning-done = 1 or Stoning-done = 2)
;    [
;     jump speed
;     stop
;    ]
